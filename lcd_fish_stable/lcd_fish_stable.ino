...
float checkLiters() {
    //used for testing graphics w/out meter: !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    //pulses = pulses+2;
...
void loop() {
  ...
  //screen size : 320 x 480
    int x, y;
    int w;
    // check if within usage
    if (checkLiters() < 0.88) //CHANGE THRESHOLD HERE
    {
     //snail
    ...
    tft.drawLine(180,280,200,260,0xFC10 ^ 0xFFFF);
      ...
      if (current_liters > 0.88) //CHANGE THRESHOLD HERE
        break;

      // ANIMATION /////////////////////////////////
      tft.drawRGBBitmap(w, 100+(0.4)*w+(-3*(w%2)), fish2, 88, 56);
      // (horizaontal pos, vertical pos, bmp, x size, y size)
      tft.fillRect(w+45, 95+(0.4)*w+(-3*(w%2)), 5,5, WATER);
      // correcting^
      //second fish
